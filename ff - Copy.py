...
def update(self, true_label, predicted_label):
        """
        Update the confusion matrix with a new prediction
        Args:
            true_label (int): Ground truth label (0 for Mask, 1 for NoMask)
            predicted_label (int): Predicted label (0 for Mask, 1 for NoMask)
        """
...
def plot_confusion_matrix(self, save_path=None):
        ...
        if len(self.true_labels) == 0:
            print("No predictions collected yet")
            return
...
def get_metrics(self):
        """
        Calculate detailed metrics including accuracy, precision, recall, F1 score, FAR, and FRR
        Returns:
            dict: Dictionary containing all metrics and confusion matrix values
        """
        if len(self.true_labels) == 0:
            return None
...
def print_metrics(self):
        ...
        for class_name, class_metrics in metrics.items():
            ...
            cm_values = class_metrics['confusion_matrix_values']
            print(f"Confusion Matrix Values:")
            print(f"True Positives (TP): {cm_values['tp']}")
            print(f"False Positives (FP): {cm_values['fp']}")
            print(f"True Negatives (TN): {cm_values['tn']}")
            print(f"False Negatives (FN): {cm_values['fn']}")
            
            # Print calculated metrics
            print(f"\nCalculated Metrics:")
            m = class_metrics['metrics']
            print(f"Accuracy: {m['accuracy']:.2f}")
            print(f"Precision: {m['precision']:.2f}")
            print(f"Recall: {m['recall']:.2f}")
            print(f"F1-Score: {m['f1_score']:.2f}")
            print(f"False Acceptance Rate (FAR): {m['far']:.2f}")
            print(f"False Rejection Rate (FRR): {m['frr']:.2f}")
